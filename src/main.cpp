...
unsigned int nTargetSpacing = 1 * 180; // 3 minute blocks
static const int64_t nInterval = nTargetTimespan / nTargetSpacing;
static const int64_t nDiffChangeTarget = 1;

unsigned int nStakeMinAge = 4 * 60 * 60; // 4 hours
unsigned int nStakeMaxAge = 60 * 60 * 24 * 128;  // 30 days
...
const string strMessageMagic = "HOST Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees) 
{
    ...
    else if (pindexBest->nHeight+1 < 32)
    {
      nSubsidy = 0 * COIN;
      return nSubsidy + nFees;
    }
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "HOST", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "1451691120 - Spliffcoin - Alltime High Crypto Currency";
        CTransaction txNew;
        txNew.nTime = 1451691120;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x778edf5086ff5887ed766873287beb4e32e734070089d8dabaa09237cf42ac85"));
...
unsigned char pchMessageStart[4] = { 0xb2, 0xb1, 0xa2, 0xa3 };
